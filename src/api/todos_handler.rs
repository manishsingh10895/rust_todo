...
pub async fn delete_todo(
    ...
    let _ = web::block(move || remove_todo(pool, params.into_inner().as_str())).await??;
...
fn remove_todo(pool: web::Data<Pool>, todo_id: &str) -> Result<(), TodoApiError> {
    ...
    match uuid::Uuid::parse_str(todo_id) {
        Ok(uid) => {
            let update_count =
                diesel::delete(todos.filter(id.eq(uuid::Uuid::parse_str(todo_id).unwrap())))
                    .execute(conn)?;
        ...
        Err(e) => {
            eprintln!("Error while deleting Todo");
...
fn insert_new_todo(
    ...
    let new_todo = Todo::from_title(todo.title, uuid::Uuid::parse_str(requester_id).unwrap());
...
fn update_todo_completeness(
    ...
    match uuid::Uuid::parse_str(todo_id) {
        Ok(uid) => {
            let update_count: usize = diesel::update(todos)
                .filter(id.eq(uid))
